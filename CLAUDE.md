...
- `other` 枠は移動カードを供給する用途 (今後の新カード追加先)。
...
type HandLogReason =
  ...
  | 'attack' | 'function' | 'calc'             // メインアクションでの消費
...
7. **メインアクション受信**: `attack` / `function` / `skip` を待つ (タイムアウト時は `skip` 自動投入)
8. **解決処理** (`lib/game-logic.ts` の `resolveActions`):
   - 攻撃: 弾を生成、手札から消費 (HandLog: remove × 1)
   - 関数: 式をバリデーション・評価し `FunctionCurve` を生成、フィールドに追加 (HandLog: remove × 消費カード数)
...
13. **情報の非対称性**: 相手の手札の中身・移動カードによる即時移動は秘匿。見せるのは枚数・**次ターン補充プレビュー (nextDraw)**・残り関数回数・ターン開始位置・公開済みのフィールド要素
